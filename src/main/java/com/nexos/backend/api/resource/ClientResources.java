...
import com.nexos.backend.api.services.ClientService;
import com.nexos.backend.api.vo.ClientVO;

@RestController
@RequestMapping("/nexos-api/client")
public class ClientResources {
	...
	public ResponseEntity findAll() {
		try {
			return new ResponseEntity<List<ClientVO>>(clientService.findAll(), HttpStatus.OK);
	...
	public ResponseEntity findById(@PathVariable Integer id) {
		try {
			return new ResponseEntity<ClientVO>(clientService.findById(id), HttpStatus.OK);
		}catch(NexosServiceException e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(e.getCode()));
		}catch(Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(MessageCode.ERROR_SISTEMA));
	...
	@PostMapping("/create")
	public ResponseEntity create(@RequestBody ClientVO clientVO) {
		try {
			clientVO = clientService.create(clientVO);
			return new ResponseEntity<ClientVO>(clientVO, HttpStatus.OK);
		}catch(NexosServiceException e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(e.getCode()));
		}catch(Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(MessageCode.ERROR_SISTEMA));
