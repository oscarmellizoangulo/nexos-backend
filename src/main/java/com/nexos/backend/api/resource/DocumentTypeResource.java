...
import com.nexos.backend.api.services.DocumentTypeService;
import com.nexos.backend.api.vo.DocumentTypeVO;

@RestController
@RequestMapping("/nexos-api/documenttype")
public class DocumentTypeResource {
	...
	public ResponseEntity findAll() {
		try {
			return new ResponseEntity<List<DocumentTypeVO>>(documentTypeService.findAll(), HttpStatus.OK);
	...
	public ResponseEntity findById(@PathVariable Integer id) {
		try {
			return new ResponseEntity<DocumentTypeVO>(documentTypeService.findById(id), HttpStatus.OK);
		}catch(NexosServiceException e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(e.getCode()));
		}catch(Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(MessageCode.ERROR_SISTEMA));
	...
	@PostMapping("/create")
	public ResponseEntity create(@RequestBody DocumentTypeVO documentTypeVO) {
		try {
			documentTypeVO = documentTypeService.create(documentTypeVO);
			return new ResponseEntity<DocumentTypeVO>(documentTypeVO, HttpStatus.OK);
		}catch(NexosServiceException e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(e.getCode()));
		}catch(Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(MessageCode.ERROR_SISTEMA));
