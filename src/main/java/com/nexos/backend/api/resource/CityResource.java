...
import com.nexos.backend.api.services.CityService;
import com.nexos.backend.api.vo.CityVO;

@RestController
@RequestMapping("/nexos-api/city")
public class CityResource {
	...
	public ResponseEntity findAll() {
		try {
			return new ResponseEntity<List<CityVO>>(cityService.findAll(), HttpStatus.OK);
	...
	public ResponseEntity findById(@PathVariable Integer id) {
		try {
			return new ResponseEntity<CityVO>(cityService.findById(id), HttpStatus.OK);
		}catch(NexosServiceException e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(e.getCode()));
		}catch(Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(MessageCode.ERROR_SISTEMA));
	...
	@PostMapping("/create")
	public ResponseEntity create(@RequestBody CityVO cityVO) {
		try {
			cityVO = cityService.create(cityVO);
			return new ResponseEntity<CityVO>(cityVO, HttpStatus.OK);
		}catch(NexosServiceException e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(e.getCode()));
		}catch(Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(MessageCode.ERROR_SISTEMA));
