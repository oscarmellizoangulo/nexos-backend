...
import com.nexos.backend.api.services.ContactTypeService;
import com.nexos.backend.api.vo.ContactTypeVO;

@RestController
@RequestMapping("/nexos-api/contacttype")
public class ContactTypeResource {
	...
	public ResponseEntity findAll() {
		try {
			return new ResponseEntity<List<ContactTypeVO>>(contactTypeService.findAll(), HttpStatus.OK);
	...
	public ResponseEntity findById(@PathVariable Integer id) {
		try {
			return new ResponseEntity<ContactTypeVO>(contactTypeService.findById(id), HttpStatus.OK);
		}catch(NexosServiceException e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(e.getCode()));
		}catch(Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(MessageCode.ERROR_SISTEMA));
	...
	@PostMapping("/create")
	public ResponseEntity create(@RequestBody ContactTypeVO contactTypeVO) {
		try {
			contactTypeVO = contactTypeService.create(contactTypeVO);
			return new ResponseEntity<ContactTypeVO>(contactTypeVO, HttpStatus.OK);
		}catch(NexosServiceException e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(e.getCode()));
		}catch(Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(MessageCode.ERROR_SISTEMA));
