...
import com.nexos.backend.api.services.ServiceTypeService;
import com.nexos.backend.api.vo.ServiceTypeVO;

@RestController
@RequestMapping("/nexos-api/servicetype")
public class ServiceTypeResource {
	...
	public ResponseEntity findAll() {
		try {
			return new ResponseEntity<List<ServiceTypeVO>>(serviceTypeService.findAll(), HttpStatus.OK);
	...
	public ResponseEntity findById(@PathVariable Integer id) {
		try {
			return new ResponseEntity<ServiceTypeVO>(serviceTypeService.findById(id), HttpStatus.OK);
		}catch(NexosServiceException e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(e.getCode()));
		}catch(Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(MessageCode.ERROR_SISTEMA));
	...
	@PostMapping("/create")
	public ResponseEntity create(@RequestBody ServiceTypeVO serviceTypeVO) {
		try {
			serviceTypeVO = serviceTypeService.create(serviceTypeVO);
			return new ResponseEntity<ServiceTypeVO>(serviceTypeVO, HttpStatus.OK);
		}catch(NexosServiceException e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(e.getCode()));
		}catch(Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(MessageCode.ERROR_SISTEMA));
