...
import com.nexos.backend.api.services.PersonService;
import com.nexos.backend.api.vo.PersonVO;

@RestController
@RequestMapping("/nexos-api/person")
public class PersonResource {
	...
	public ResponseEntity findAll() {
		try {
			return new ResponseEntity<List<PersonVO>>(personService.findAll(), HttpStatus.OK);
	...
	public ResponseEntity findById(@PathVariable Integer id) {
		try {
			return new ResponseEntity<PersonVO>(personService.findById(id), HttpStatus.OK);
		}catch(NexosServiceException e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(e.getCode()));
		}catch(Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Message.getMessage(MessageCode.ERROR_SISTEMA));
